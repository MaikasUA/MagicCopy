...
namespace MagicCopy
{
    public partial class Magic_Copy : Form
    {
        ...
        private void English_CheckedChanged(object sender, EventArgs e) //English Localization
        {
            ...
            helpInfoToolStripMenuItem.Text = "\"Catalog\"  Help";
            FastTranslation();
        }
        ...
        private void SaveSetting(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {   AutoLoading.Checked.ToString(), source_way, one_way, two_way,
                    ...
                    Ukrainian.Checked.ToString(), SaveText
                };
        ...
        private void LoadingSetting()
        {
            ...
            try
            {
                ...
                for (byte i = 1; i < readSetting.Length - 1; i++) //11 строка язык
                {
                    try
                    {
                        if (i < 11) WayAndKey[i].Text = readSetting[i];
                        else
                        {
                            English.Checked = Convert.ToBoolean(readSetting[i]); j++; i++;
                            Russian.Checked = Convert.ToBoolean(readSetting[i]); j++; i++;
                            Ukrainian.Checked = Convert.ToBoolean(readSetting[i]); j++; i++;
                        }
                    }
                    catch (Exception)
                    {
        ...
        private void WayAndText()
        {
            WayAndKey = new Control[]
            {
                ...
                NewKey_Two, NewKey_Three, NewKey_Four, English, Russian, Ukrainian
            };
        }
                        ...
                        foreach (var info in newImageTag.ImageTags)
                        {
                            if (KeyWordScan[0] != "Click" || KeyWordScan[i] == info.TAG || KeyWordScan[i] == info.ID)
                            {
                                ...
                                if (SUCCESS)
                                {
                                    ...
                                    //PicBox.Tag = file.Name; //Тэг файла
        ...
        private void GetSetFileTag(string way, string info)
        {
            ...
            {                
                ...
                MegaLink = ArtID.Length == 0 ? ArtID = "Arts ID not found" : "https://derpibooru.org/" + ArtID,
                //ImageWay = ways
            };
        ...
        {
            string[] HOLE = new string[] { "Click" };
            ImageTagLoad imageTagLoad = new ImageTagLoad();
            imageTagLoad.loadingtag(ID, SelectFileWay, out string Tag, HOLE, out HOLE);
            if (Tag == "Fail" || Tag == "") { TagFiles.Text = ""; }
            else { TagFiles.Text = Tag; }
        }

        private void FoundFileInJson(string[] scan, out string [] WAYs)
        {
            ImageTagLoad imageTagLoad = new ImageTagLoad();
            imageTagLoad.loadingtag("", "", out string Tag, scan, out scan);
            WAYs = scan;
        }
